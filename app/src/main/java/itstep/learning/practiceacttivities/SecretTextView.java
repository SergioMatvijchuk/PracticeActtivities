...
public class SecretTextView extends androidx.appcompat.widget.AppCompatTextView {
    private String mTextString; //строка которая отображается
    private SpannableString mSpannableString;   //строка с использованием строка с использованием SpannableString, что позволяет применять стили к отдельным частям текста (в данном случае, для изменения цвета).

    private double[] mAlphas; // массив значений альфа-канала (прозрачности) для каждой буквы текста.
    private MutableForegroundColorSpan[] mSpans; //массив объектов MutableForegroundColorSpan, которые управляют цветом каждой буквы.
    private boolean mIsVisible; //определяет, видим ли текст.
    private boolean mIsTextResetting = false; // флаг, чтобы избежать бесконечных перерисовок во время обновления текста.
    private int mDuration = 2500; //продолжительность анимации.

    ValueAnimator animator;  // объект ValueAnimator, который анимирует изменение прозрачности (альфа-канала) текста.
    ...
    void resetSpannableString(double percent){
